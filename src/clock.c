...
static void add_clock(const char** names, const char** units,
                      const char* name, const char* unit,
                      int* clock, int nclocks)
{
  DECLARE_CCTK_PARAMETERS;
  if (!define_statistics_clocks) {
...
void PAPI_register_clock(CCTK_ARGUMENTS)
{
  ...
  int clock = 0;
  add_clock(counter_names, counter_units, "P_time", "sec",
            &clock, num_cactus_clock_vals);
  add_clock(counter_names, counter_units, "P_cycles", "Gcycles",
            &clock, num_cactus_clock_vals);
  add_clock(counter_names, counter_units, "P_count", "Gevents",
            &clock, num_cactus_clock_vals);
  for (int i=0; i<num_events; ++i) {
    ...
    add_clock(counter_names, counter_units, eventname, "Gevents",
              &clock, num_cactus_clock_vals);
  }
